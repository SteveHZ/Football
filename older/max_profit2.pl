...
my %markets = (
	"max_profit"	=> Football::Team_Hash2->new (
		func 		=> \&straight_win,
		fixtures	=> $fixtures
	),
...
for my $market (keys %markets) {
	my $filename = "$data->{out_path}$market".'_'."$data->{model_type}2.xlsx";
	my $team_hash = $markets{$market};
	my $sorted = $team_hash->sort ();

	print "\nWriting $filename...";
	my $writer = Football::Spreadsheets::Max_Profit2->new ( filename => $filename, euro => $euro );
...
Calculate max profit2 spreadsheet
This version uses Team_Hash2 and Max_Profit2 to calculate winnings
given a strategy of starting with £2.00 then applying half of the
balance each time to a given team to win
