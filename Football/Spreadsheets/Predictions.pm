...
sub do_fixtures_header {
	...
	$worksheet->set_column ($_, 3) for ('A:G','J:P','Q:Q');
...
sub do_league_places_header {
	...
	$worksheet->set_column ($_, 3) for ('C:C', 'F:G', 'I:I', 'K:K');
	$worksheet->set_column ($_, 8) for ('H:H', 'J:J', 'L:L');
...
sub do_goal_diff_header {
	...
	$worksheet->set_column ($_, 3) for ('C:D', 'F:G', 'I:I', 'K:K');
	$worksheet->set_column ($_, 8) for ('E:E', 'H:H', 'J:J', 'L:L');
...
sub do_recent_draws_header {
	...
	$worksheet->set_column ($_, 3) for ('B:B', 'E:E');
	$worksheet->set_column ($_, 25) for ('C:D', 'I:I');
