...
sub do_teams {
	...
	for my $team (@$sorted) {
		...
		my $row = 0;
		$worksheet->write ($row, 0, "DATE", $self->{bold_format});
		$worksheet->write ($row, 1, "OPPONENT", $self->{bold_format});
		$worksheet->write ($row, 2, "H A", $self->{bold_format});
		$worksheet->write ($row, 3, "RESULT", $self->{bold_format});
		$worksheet->write ($row++, 4, "SCORE", $self->{bold_format});
...
sub do_table {
	...
	for my $title (@titles) {
		$worksheet->write ($title->{col}, $title->{title}, $self->{bold_format});
	}
